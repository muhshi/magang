...
- **Widget Kalender Dashboard**: Menambahkan widget kalender sebulan penuh di dashboard admin (`saade/filament-fullcalendar`). Kalender ini otomatis memuat dan mewarnai dua jenis data: **Hari Libur Nasional** (diambil dari API dengan warna merah) dan **Agenda Cuti Disetujui** (diambil dari database `Leave` dengan warna biru).