...
- **Restore Disappeared Widgets**: Meregenerasi seluruh *permissions* Filament Shield melalui sinkronisasi ulang agar widget-widget admin (seperti `StatsOverview`, `RecentActivityTable`, dsb.) kembali muncul. Hal ini mencegah *fatal error* akibat hilangnya fallback *method* `canAccess()` di Filament v5 pada widget yang tidak memiliki *permission*.
...
- **Remove Regulasi & Google SSO**: Menghapus total modul `Aturan` (tabel database, Model, Policy, Resource) yang menangani WFA/Banned dan mencopot pengecekannya dari logika Presensi. Fitur login via Google Identity Services (`GoogleAuthController`, rute, *custom view* pada Filament Admin) juga dihapus secara bersih, mengembalikan *form* login sistem seperti standar bawaan Filament.
- **Fix Logout Button**: Memperbaiki tombol "Log out" yang sebelumnya tidak berfungsi di Menu Profil karena salah tertimpa oleh rute kustom kosong. Logika *logout* sudah dikembalikan sepenuhnya ke bawaan sistem keamanan Filament.